...
#pragma mark --手动上下轮播
- (void)autoNextPage:(BOOL)UpOrNo {
    
    if (UpOrNo) {
        //        NSLog(@"slef.page==%ld",self.page);
        self.page ++;
        //        NSLog(@"slef.page==%ld",self.page);
        switch (self.orientation) {
            case NewPagedFlowViewOrientationHorizontal:{
            ...
            case NewPagedFlowViewOrientationVertical:{
                ...
                //动画
                if(self.scrollViewWillBeganDraggingBlock){
                    self.scrollViewWillBeganDraggingBlock();
                }
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if(self.scrollViewWillStopDraggingBlock){
                        self.scrollViewWillStopDraggingBlock();
                    }
                });
...
//        NSLog(@"slef.page==%ld",self.page);
        switch (self.orientation) {
            case NewPagedFlowViewOrientationHorizontal:{
                ...
                [_scrollView setContentOffset:CGPointMake(self.page * _pageSize.width, 0) animated:NO];
                break;
            }
            case NewPagedFlowViewOrientationVertical:{
                ...
                ////                //减少的循环问题，故意设置两组图片，这里显示18-8（8会停顿一次），将9出现腹部的时候，自动跳转到14上，背后做效果
                //                if(self.page == 2){
        ...
        case NewPagedFlowViewOrientationVertical:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.height) {
                    ...
                    //微调
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(topBottomInset, leftRightInset*10+8, topBottomInset, -leftRightInset*2));
                    cell.mainImageView.frame = cell.bounds;
                    
                    if (iPhone5) {
                        cellCoverViewCenterX = 70;
                        cell.frameImageView.frame = CGRectMake(cell.bounds.origin.x - 4 , cell.bounds.origin.y - 5, cell.bounds.size.width + 8, cell.bounds.size.height + 10);
                    }else if (iPhone6){
                        cellCoverViewCenterX = 95;
                        //微调
                        cell.frameImageView.frame = CGRectMake(cell.bounds.origin.x - 4 , cell.bounds.origin.y - 6, cell.bounds.size.width + 10, cell.bounds.size.height + 11);
                    }else{
                        cellCoverViewCenterX = 100;
                        cell.frameImageView.frame = CGRectMake(cell.bounds.origin.x - 4 , cell.bounds.origin.y - 6, cell.bounds.size.width + 10, cell.bounds.size.height + 11);
                    }
                    
                    cell.coverView.frame = cell.bounds;
                    
                    if (cell.coverView.center.x < cellCoverViewCenterX) {
                        
                        cell.coverView.alpha = 0.3;
                        cell.frameImageView.hidden = YES;
                        cell.mainImageView.transform = CGAffineTransformScale(cell.mainImageView.transform, 1, 0.75);
                        cell.coverView.transform = CGAffineTransformScale(cell.coverView.transform, 1, 0.75);
                    ...
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(self.topBottomMargin ,self.leftRightMargin*10, self.topBottomMargin, -self.leftRightMargin*2));
    ...
    if (self.isCarousel) {
        if (self.orginPageCount > 1) {
            switch (self.orientation) {
                ...
                case NewPagedFlowViewOrientationVertical:
                {
//                    NSLog(@"当前下标 === %f === %f === %f",scrollView.contentOffset.y / _pageSize.height  ,scrollView.contentOffset.y,_pageSize.height);
                    if (scrollView.contentOffset.y / _pageSize.height >= 2 * self.orginPageCount) {
                        ...
                        self.page = self.orginPageCount;
//                        NSLog(@"福了 === %ld",self.orginPageCount);
                    }
                    
                    if (scrollView.contentOffset.y / _pageSize.height <= self.orginPageCount - 1 + 0.2) {
                        [scrollView setContentOffset:CGPointMake(0, (2 * self.orginPageCount - 1) * _pageSize.height) animated:NO];
                        //减一，控制下标
                        self.page = 2 * self.orginPageCount - 1;
                        
//                        NSLog(@"沉了 === %ld",self.orginPageCount);
                    }
