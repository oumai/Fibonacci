...
- (id)init {
    ...
    [session requestRecordPermission:^(BOOL granted) {
        if (granted) {
            ...
            NSArray *zhArray = [[self getPreferredLanguage] componentsSeparatedByString:@"-"];
            NSString *title = @"";
            NSString *message = @"";
            NSString *cancel = @"";
            if ([zhArray[0] isEqualToString:@"zh"])
            {
                title = @"麦克风访问被拒绝";
                message = @"此应用程序需要访问您的设备的麦克风。\n\n请在设置中启用麦克风访问这个应用程序,\n\n设置 > 隐私 >麦克风";
                cancel = @"取消";
            }
            else
            {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                UIAlertAction *canleAction = [UIAlertAction actionWithTitle:cancel style:UIAlertActionStyleCancel handler:nil];
    ...
    NSError *nsError;
    [session setPreferredSampleRate:rate error:&nsError];
    if( nsError != nil ){}
	return self;
}
