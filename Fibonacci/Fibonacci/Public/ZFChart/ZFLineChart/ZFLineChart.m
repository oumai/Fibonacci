...
- (void)drawCircle{
    ...
    if ([subObject isKindOfClass:[NSString class]]) {
        for (NSInteger i = 0; i < self.genericAxis.xLineValueArray.count; i++) {
            ...
            circle.strokeColor = circle.circleColor;
            if (!self.genericAxis.isDefaulfShow && i == self.genericAxis.xLineValueArray.count -1)
            {
                circle.circleColor = [UIColor whiteColor];
                circle.strokeColor = _colorArray.firstObject;
            }
...
//    NSLog(@"一共 %li",self.circleArray.count);
    ...
    if ([subObject isKindOfClass:[ZFCircle class]]) {
        for (NSInteger i = 0; i < self.circleArray.count; i++)
        {
            if (i == self.circleArray.count-1) {
                [self addValueLabel:self.circleArray index:self.circleArray.count-1 colorIndex:0 valueArray:self.genericAxis.xLineValueArray valuePosition:(kChartValuePosition)[self.valuePositionArray.firstObject integerValue] hidden:NO];
            }
            else
            {
                [self addValueLabel:self.circleArray index:i colorIndex:0 valueArray:self.genericAxis.xLineValueArray valuePosition:(kChartValuePosition)[self.valuePositionArray.firstObject integerValue] hidden:!self.isDefaulfShow];
            }
        ...
        for (NSInteger i = 0; i < self.circleArray.count; i++) {
            ...
            for (NSInteger j = 0; j < subArray.count; j++) {
                [self addValueLabel:subArray index:j colorIndex:i valueArray:self.genericAxis.xLineValueArray[i] valuePosition:(kChartValuePosition)[self.valuePositionArray[i] integerValue] hidden:NO];
            }
        }
    }
}
 ...
 */
- (void)addValueLabel:(NSMutableArray *)circleArray index:(NSInteger)index colorIndex:(NSInteger)colorIndex valueArray:(NSMutableArray *)valueArray valuePosition:(kChartValuePosition)valuePosition hidden:(BOOL)hidden{
    //当前的圆
...
//    popoverLabel.textColor = ZFLightGray;
    ...
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(popoverLongPress:)];
    longPress.delegate = self;
    [popoverLabel addGestureRecognizer:longPress];
    ...
    //_valueOnChartPosition为圆环上方
        ...
        popoverLabel.arrowsOrientation = kPopoverLaberArrowsOrientationOnBelow;
        popoverLabel.center = CGPointMake(circle.center.x, circle.center.y - _valueCenterToCircleCenterPadding*0.5);
...
#pragma mark - 长按事件
-(void)circleLongPress:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if ([self.delegate respondsToSelector:@selector(lineChart:didLongPressSelectCircleAtLineIndex:circleIndex:)]) {
            if ([gesture.view isKindOfClass:[ZFCircle class]]) {
                ZFCircle *label = (ZFCircle *) gesture.view;
                [self.delegate lineChart:self didLongPressSelectCircleAtLineIndex:label.lineIndex circleIndex:label.circleIndex];
            }
        }
    }
...
-(void)popoverLongPress:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if ([self.delegate respondsToSelector:@selector(lineChart:didLongPressSelectPopoverLabelAtLineIndex:circleIndex:)]) {
            if ([gesture.view isKindOfClass:[ZFPopoverLabel class]]) {
                ZFPopoverLabel *label = (ZFPopoverLabel *) gesture.view;
                [self.delegate lineChart:self didLongPressSelectPopoverLabelAtLineIndex:label.groupIndex circleIndex:label.labelIndex];
            }
        }
    }
}
...
// 显示值
    ...
    if (!self.genericAxis.isDefaulfShow) {
        [self.genericAxis bringCircleToFront];
    }
...
{
    self.genericAxis.isDefaulfShow = isDefaulfShow;
}
@end
